...
session$userData$codematch = FALSE                   # flag for where we are in the process
...
output$pageStub <- renderUI({
   ...
   if(!session$userData$codematch) {                 # step 1, get the username
      if(session$userData$user$username =="") {
         tagList(
            fluidRow(
               column(4, offset=4,
      ...
      } else {                                       # step 2, and send and receive code
   ...
   } else {                                          # step 3, get the new password
