...
output$pageStub <- renderUI({
   ...
   if(session$userData$user$sp && !session$userData$user$emailverified) {   # so we need to verify email address?
      ...
      if(session$userData$user$sp) {  # >0 = TRUE = logged in = updating profile
         pageText = tagList(
      ...
      } else {                        # not logged in; initial registration
         pageText = tagList(
...
# need to deal with checking for valid email addresses
...
observeEvent(input$register_btn, {
   ...
   if(alertText=="") {                                             # we inherit a blank session$userData$user from app.R startup
      session$userData$user$username <- username
...
#     rv$limn = rv$limn + 1                           # this inits fields to original values; leave user-entered text
   }
})
...
observeEvent(input$fixemail_btn, {
   if(input$email!=session$userData$user$email) {           # if ==, we already have it
      session$userData$user$email <- input$email
      session$userData$user$emailverified <- FALSE          # This will take us back to verification on render
      userSave(session$userData$user)
   }
   rv$limn = rv$limn + 1                                    # This will send another email with a new code.
})
